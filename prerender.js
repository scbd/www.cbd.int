...
prerender.shouldShowPrerenderedPage = function(req) {
  ...
  //console.log('extension req.url',req.originalUrl) ;
  //console.log('is extention?',prerender.extensionsToIgnore.some(function(extension){return req.originalUrl.toLowerCase().indexOf(extension) !== -1;}));
  if(prerender.extensionsToIgnore.some(function(extension){return req.originalUrl.toLowerCase().indexOf(extension) !== -1;})) return false;
...
prerender.buildApiUrl = function(req) {
  var prerenderUrl = prerender.getPrerenderServiceUrl(req);
