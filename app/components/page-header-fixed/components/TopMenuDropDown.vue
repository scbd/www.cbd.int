...
function initState(){
  if(!Array.isArray(this.opts.siteNavElms)) return

  const length = this.opts.siteNavElms.length
...
function open (index){ 
 this.set(index, true)
  // debounce( () => {this.set(index, true)}, 100) 
}

function close (index){ 
  this.set(index, false)
 // debounce(() => this.set(index, false), 100) 
}
