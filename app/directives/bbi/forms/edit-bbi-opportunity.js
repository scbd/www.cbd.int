		...
		return {
			...
			link: function($scope) {
				...
				$scope.updateWebsites = function(name, value) {
					...
					var site = _.find($scope.document.relevantDocuments, {
						'name': name
					});
					if (site)
						if (value)
							...
							delete($scope.document.relevantDocuments[_.findIndex($scope.document.relevantDocuments, {
								'name': name
							})]);

					else
						$scope.document.relevantDocuments.push({
							name: name,
							url: value,
							type: 'link'
						});
