	...
	return {
		...
		link : function($scope)
		{
			...
			$scope.options  = {//5FF88869-FD8C-438F-9F77-66AB1CFB3BD4
				...
				jurisdictions:	function() { return $http.get("/api/v2013/thesaurus/domains/50AC1489-92B8-4D99-965A-AAE97A80F38E/terms",	{ cache: true }).then(function (o) { return o.data; });},
				...
				assistanceTypes			: function() { return $http.get("/api/v2013/thesaurus/domains/5FF88869-FD8C-438F-9F77-66AB1CFB3BD4/terms",            { cache: true }).then(function(o){ return $filter('orderBy')(o.data, 'name'); }); },
						...
						.then(function(data){
							...
							if(data.contactOrganization && data.contactOrganization.identifier){
								    $scope.loading.org=true;
										...
										.then(function(d){
												$scope.document.organization = {identifier:d.header.identifier};
												$scope.loading.org=false;
												killContactWatch ();
										});
			...
			$scope.nextTab = function() {
				...
				if($scope.tab=='org') 			{ next = 'general';}
				if($scope.tab=='general') 	{ next = 'details';   }
				if($scope.tab=='details' 	) { next = 'linkages'; }
				if($scope.tab=='linkages') 		{ next = 'review';  $scope.validate();}
			...
			$scope.prevTab = function() {
				...
				if($scope.tab=='review' ) { prev = 'linkages'; 	 }
				if($scope.tab=='linkages'	) { prev = 'details';  }
				if($scope.tab=='details'	) { prev = 'general';}
				if($scope.tab=='general'	) { prev = 'org';}
			...
			function loadDraftOrNew(){
				...
				var fields={history:0};

				var sort = {'meta.modifiedOn':-1};
					...
					if($scope.editForm.$error && $scope.editForm.$error.required && $scope.editForm.$error.required.length){
							...
							for(var i=0; i<$scope.editForm.$error.required.length;i++)
									if($scope.editForm.$error.required[i].$name!=='linkForm' && $scope.editForm.$error.required[i].$name)
										$scope.validationReport.errors.push({code:'Error.Mandatory',property:$scope.editForm.$error.required[i].$name});
